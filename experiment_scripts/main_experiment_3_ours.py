...
for num in range(10):
    ...
    for it, (start, end) in enumerate(zip(x0, xf)):
        ...
        try:
            ...
            traj_ori = planner.path_index_to_path_point(post_path)

            # if world_frame:
            #     traj = nerfwrapper.ns_frame_to_data_frame(torch.from_numpy(traj[..., :3]).to(device, dtype=torch.float32)).squeeze().cpu().numpy()

            traj_list.append(traj.tolist())
            traj_ori_list.append([point.tolist() for point in traj_ori])
    ...
    fp = f'result/experiment/experiment3/ours/{exp_name}/data_seed{SEED}_exnum{num+1}.json'
