...
from pipeline_scripts.Astar_simple import *

# Perform coarse path planning
def astar3D(field, source, target, feasible):

    source = tuple(map(np.int32, source)) # Find nearest grid point and find its index
    target = tuple(map(np.int32, target))

    astar = AStarSimple(tuple(source), target, field)
    inds, closed_list_astar, opened_list_astar = astar.run()
    inds = np.array(inds)
